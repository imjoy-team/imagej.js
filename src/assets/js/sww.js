...
SimpleOfflineCache.prototype.onFetch = function soc_onFetch(request, response) {
    ...
    if(request.method === 'HEAD') { opt.ignoreMethod = true }
    var clone
    // if(request.method === 'HEAD') {
    //     clone = modifyRequest(request)
    // }
    // else{
        clone = request.clone();
    // }
    return cache.match(clone, opt).then(function(res) {
        if (res) {
...
function modifyRequest(urlOrReq) {
  ...
  return new Request(url, {
    cache, credentials, headers, integrity, method: met,
    mode, redirect, referrer, referrerPolicy, body
  })
}

SimpleOfflineCache.prototype.fetchAndCache =
function sc_fetchAndCache(request, cache) {
    ...
    // if (parseInt(response.status) < 400) {
        if(request.mode === 'navigate'){
            ...
            // const req = modifyRequest(request);
